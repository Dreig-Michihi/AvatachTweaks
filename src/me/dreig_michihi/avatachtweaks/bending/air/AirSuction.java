...
public class AirSuction extends AirAbility implements AddonAbility {
	...
	private double groundJumpCooldownCoefficient;
	private double groundJumpPower;
	private double airJumpCooldownCoefficient;
	private double airJumpPower;
	private long simpleClickCooldown;
	...
	public AirSuction(final Player player) {
		...
		if(!getConfig().getBoolean(TweaksConfig.getConfigPath(this,"CanBeUsedOnSpout")))
			if (hasAbility(player, AirSpout.class) || hasAbility(player, WaterSpout.class)) {
				return;
			}
		...
		this.groundJumpCooldownCoefficient =
				getConfig().getDouble(TweaksConfig.getConfigPath(this,"GroundJumpCooldownCoefficient"));
		this.groundJumpPower =
				getConfig().getDouble(TweaksConfig.getConfigPath(this,"GroundJumpPower"));
		this.airJumpCooldownCoefficient =
				getConfig().getDouble(TweaksConfig.getConfigPath(this,"AirJumpCooldownCoefficient"));
		this.airJumpPower =
				getConfig().getDouble(TweaksConfig.getConfigPath(this,"AirJumpPower"));
		this.simpleClickCooldown =
				getConfig().getLong(TweaksConfig.getConfigPath(this,"SimpleClickCooldown"));
	...
	public static void shoot(final Player player) {
		...
		if (CoreAbility.hasAbility(player, AirSuction.class)) {
			...
			suc.cooldown=suc.simpleClickCooldown;
		}
	...
	public static void easyJump(Player player) {
		if (CoreAbility.hasAbility(player, AirSuction.class)) {
			CoreAbility.getAbility(player, AirSuction.class).remove();
		}
		AirSuction suc = new AirSuction(player);
		if(BendingPlayer.getBendingPlayer(player).isOnCooldown(suc))
			return;
		if(GeneralMethods.isSolid(suc.player.getLocation().getBlock().getRelative(BlockFace.DOWN))) {
			suc.cooldown*=suc.groundJumpCooldownCoefficient;
			player.setVelocity(suc.player.getEyeLocation().getDirection().multiply(-suc.groundJumpPower));
		}
		else {
			suc.cooldown*=suc.airJumpCooldownCoefficient;
			player.setVelocity(suc.player.getEyeLocation().getDirection().multiply(-suc.airJumpPower));
		}
		suc.bPlayer.addCooldown(suc);
		suc.remove();
		ParticleEffect.CLOUD.display(suc.player.getLocation(), 7, .5, 0, .5, 0.1);
	}
	public static void playEasyJumpParticles(Player player) {
		playAirbendingParticles(player.getEyeLocation().add(player.getEyeLocation().getDirection()), 1, .05, .05, .05);
	}
	...
	public void load() {
		...
		TweaksConfig.addDefault(this, "SimpleClickCooldown", 1500);
		TweaksConfig.addDefault(this, "GroundJumpPower", 1.4);
		TweaksConfig.addDefault(this, "GroundJumpCooldownCoefficient", 0.5);
		TweaksConfig.addDefault(this, "AirJumpPower", 1.5);
		TweaksConfig.addDefault(this, "AirJumpCooldownCoefficient", 0.75);
		TweaksConfig.addDefault(this, "CanBeUsedOnSpout", false);
