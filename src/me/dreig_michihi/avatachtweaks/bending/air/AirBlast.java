...
import me.dreig_michihi.avatachtweaks.bending.air.util.AirbendingManager;
...
public class AirBlast extends AirAbility implements AddonAbility {
	...
	public AirBlast(final Player player) {
		...
		if (ORIGINS.containsKey(player)) {
			...
			this.cooldown=this.simpleClickCooldown;
		}
	...
	private static void playOriginEffect(final Player player) {
		...
		} else playAirbendingParticles(origin, getSelectParticles());
	}
	...
	private void easyJump(){
		if(GeneralMethods.isSolid(this.player.getLocation().getBlock().getRelative(BlockFace.DOWN))) {
			this.cooldown *= groundJumpCooldownCoefficient;
			this.player.setVelocity(this.player.getEyeLocation().getDirection().multiply(this.groundJumpPower));
		}
		else {
			this.cooldown *= airJumpCooldownCoefficient;
			this.player.setVelocity(this.player.getEyeLocation().getDirection().multiply(this.airJumpPower));
		}
		this.bPlayer.addCooldown(this);
		this.remove();
		super.remove();
		ParticleEffect.CLOUD.display(this.player.getLocation(), 7, .5, 0, .5, 0.1);
	}
	...
	public void load() {
		...
		TweaksConfig.addDefault(this, "GroundJumpCooldownCoefficient", 0.2);
		TweaksConfig.addDefault(this, "AirJumpPower", 1.5);
		TweaksConfig.addDefault(this, "AirJumpCooldownCoefficient", 0.5);
